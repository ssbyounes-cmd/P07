...
class TournamentPlatform:
    ...
    def register_card(self, card: TournamentCard) -> str:
        ...
        count = self._category_counters.get(prefix, 0) + 1
        self._category_counters[prefix] = count
        card_id = f"{prefix}_{count:03d}"
        self.registered_cards[card_id] = card
        return card_id

    def create_match(self, card1_id: str, card2_id: str) -> dict:
        if (card1_id not in self.registered_cards or card2_id
                not in self.registered_cards):
            return {"error": "One or both cards not registered"}
        card1 = self.registered_cards[card1_id]
        card2 = self.registered_cards[card2_id]

        # Simulate match (winner will be the last one standing)
    ...
    def get_leaderboard(self) -> list:
        ...
        for card in card_rating:
            ratings_info.append(f"{index}. {card.name} - Rating: "
    ...
    def generate_tournament_report(self) -> dict:
        ...
        avg_rating = int(sum(ratings_list) / len(ratings_list)
